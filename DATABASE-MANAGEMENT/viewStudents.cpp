/* Compile using:
g++ -Wall -I/usr/include/cppconn -o odbc viewStudents.cpp -L/usr/lib -lmysqlcppconn
run:
./odbc */
#include "odbc_db.h"
#include <iostream>
...
string SchemaName = "bkj011"; // Change to your username
   ...
   if(major.compare("\\*") == 0){
	   builder.append(myDB.query("SELECT * FROM STUDENTS;"));
   }else{
	   builder.append(myDB.query("SELECT * FROM STUDENTS WHERE MAJOR = '" + major + "';"));
   }
